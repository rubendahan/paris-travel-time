...
- 🌊 **The Seine is real.** Isochrones do not swim: walking propagation honors an OSM-derived mask (water blocked, bridges open), both for the colored bands and for the initial walk from your marker.
...
**Rendering.** Started as tflmap-style circles (one per stop, radius = remaining walking distance), now a raster: the client evaluates "minutes to reach this point" on a viewport grid, paints one pixel per cell in its band color, and drops the canvas on the map as a single image overlay. Vector polygons (marching squares) came first and were dropped: Leaflet mangled the giant concave rings at render time, half a band could silently vanish. Pixels have no winding, clipping or ring-assembly bugs, and the browser's bilinear upscaling smooths the band edges for free. The walking part respects a 30 m **walkability mask** built from OpenStreetMap: water is blocked (lakes and river polygons, plus river and canal centerlines stroked at their real width), the 12 000+ pedestrian bridges of the region are carved back in, and the propagation is a multi-source Dijkstra instead of a plain distance transform. Concretely: the colors stop at the Seine and only cross it on bridges, comme il se doit. One hard-earned detail survives from the circle era: layer opacity must be applied once per layer, never per shape, otherwise alpha stacks into saturated blobs.

The engine is city-agnostic: the ingestion reads plain GTFS and the scan has no idea what a métro is, so pointing the download script at another feed does produce the same map elsewhere. The region still shows through at the edges, and moving the app would mean revisiting them: the walkability mask is built for an Île-de-France bounding box, the default feed URL and the JIT warm-up coordinates are IDFM's, and the frontend geocodes through the French national address API and opens on Châtelet.
